"""Trending-topic research from multiple live sources.
...
from pipeline.discord_notify import send_research_skip_notification
...
def fetch_gemini_grounded(gemini_api_key: str) -> SourceAttempt:
...
def fetch_hacker_news() -> SourceAttempt:
    name = "Hacker News"
    try:
        ids = requests.get(
...
def fetch_wikipedia_current() -> SourceAttempt:
...
def fetch_rss_feeds() -> SourceAttempt:
...
def fetch_youtube_popular(youtube_api_key: str | None) -> SourceAttempt:
...
def fetch_reddit_json() -> SourceAttempt:
    name = "Reddit .json"
    try:
        r = requests.get(
...
def fetch_reddit_oauth(client_id: str | None, client_secret: str | None) -> SourceAttempt:
    name = "Reddit OAuth"
...
def _pick_topic_with_gemini(api_key: str, research_context: str) -> str:
...
def _pick_topic_with_huggingface(hf_token: str, research_context: str) -> str:
...
def pick_todays_topic(
    ...
    *,
    ...
    attempts: list[SourceAttempt] = [
        fetch_gemini_grounded(gemini_api_key),
        fetch_hacker_news(),
        fetch_wikipedia_current(),
        fetch_rss_feeds(),
        fetch_youtube_popular(youtube_api_key),
        fetch_reddit_json(),
        fetch_reddit_oauth(reddit_client_id, reddit_client_secret),
    ]
    ...
    if skipped:
        ...
        try:
            send_research_skip_notification(discord_webhook_url, skip_lines)
